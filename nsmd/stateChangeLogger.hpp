...
class StateChangeLogger
{
  ...
  private:
    using StateChangeArg = std::variant<utils::bitfield256_err_code, bool>;
    ...
    StateChangeArg initializeArg(T)
    {
        ...
        {
            return utils::bitfield256_err_code{};
        }
                            ...
                            T&& arg)
    {
        stateChanged |= std::visit(
            ...
            if constexpr (std::is_same_v<StoredType,
                                         utils::bitfield256_err_code>)
            {
                return !isSuccess(arg) && !value.isBitSet(arg);
            }
