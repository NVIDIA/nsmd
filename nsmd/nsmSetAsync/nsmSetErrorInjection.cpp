    ...
    std::shared_ptr<NsmDevice> device)
{
    ...
    if (!enabledValue)
    {
        // Reported rather than thrown: a throw from a handler coroutine is
        // stored in its promise and never reaches setImpl's catch, so the
        // client would be told Success while nothing was written.
