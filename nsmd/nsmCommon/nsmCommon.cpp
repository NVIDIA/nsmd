                  ...
                  uuid_t& uuid)
{
    ...
    if (nsmDevice)
    {
        auto fallbackToMctpUuid = [&nsmDevice, &uuid]() {
        ...
        {
            lg2::info(
                "getDeviceUUID::encode_get_inventory_information_req failed, assigning mctp uuid eid={EID} rc={RC}",
                "EID", nsmDevice->getEid(), "RC", rc);
            fallbackToMctpUuid();
            co_return NSM_SW_SUCCESS;
        }
        ...
        if (rc)
        {
            ...
            if (rc == NSM_ERR_UNSUPPORTED_COMMAND_CODE)
            {
                ...
                lg2::info(
                    "getDeviceUUID::sensorIO failed with rc not supported, assigning mctp uuid eid={EID} rc={RC}",
                    "EID", nsmDevice->getEid(), "RC", rc);
                co_return NSM_SW_SUCCESS;
            }
            lg2::info(
                "getDeviceUUID::sensorIO failed, assigning mctp uuid eid={EID} rc={RC}",
                "EID", nsmDevice->getEid(), "RC", rc);
            co_return NSM_SW_SUCCESS;
        }
        ...
        if (cc == NSM_SUCCESS)
        {
            ...
            nsmDevice->deviceUuid = uuid;
            lg2::info(
                "getDeviceUUID::SendRecvNsmMsg pass assigning device uuid from getInventory command eid={EID} rc={RC}",
                "EID", nsmDevice->getEid(), "RC", rc);
            co_return NSM_SW_SUCCESS;
        }

        // Type 3 Get Inventory information is supported, but DEVICE_GUID can
        // still return a non-success completion code. Use MCTP UUID for any
        ...
        {
            lg2::info(
                "getDeviceUUID::Got non-success CC value, assigning mctp uuid eid={EID} cc={CC} reasonCode={REASONCODE} rc={RC}",
                "EID", nsmDevice->getEid(), "CC", cc, "REASONCODE", reason_code,
                "RC", rc);
        }
        fallbackToMctpUuid();
