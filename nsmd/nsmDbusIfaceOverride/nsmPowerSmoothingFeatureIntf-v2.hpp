...
class OemPowerSmoothingFeatIntfV2 : public OemPowerSmoothingFeatIntf
{
  public:
    ...
    requester::Coroutine togglePowerSmoothingOnDevice(
        bool featureEnabled, AsyncOperationStatusType* status,
        [[maybe_unused]] std::shared_ptr<NsmDevice> device)
    {
        SensorManager& manager = SensorManager::getInstance();
        auto eid = manager.getEid(device);
        ...
        if (cc == NSM_SUCCESS && rc == NSM_SW_SUCCESS)
        {
            ...
            {
                co_await OemPowerSmoothingFeatIntf::
    ...
    requester::Coroutine toggleImmediateRampDownOnDevice(
        bool ramdownEnabled, AsyncOperationStatusType* status,
        [[maybe_unused]] std::shared_ptr<NsmDevice> device)
    {
        SensorManager& manager = SensorManager::getInstance();
        auto eid = manager.getEid(device);
        ...
        [[maybe_unused]] std::shared_ptr<NsmDevice> device)
    {
        ...
        const auto rc = co_await toggleImmediateRampDownOnDevice(
            *ramdownEnabled, status, device);
