...
TEST_F(NsmDebugTokenUnifiedMultiRecordTest, AllRecordsInstallSuccess)
{
    ...
    // 3 records, one chunk each, plus the queryToken refresh after success.
    EXPECT_CALL(*mockDevice, postPatchIO)
        .WillOnce(mockPostPatchIO(createInstallTokenResponse(NSM_SUCCESS, 0)))
        .WillOnce(mockPostPatchIO(createInstallTokenResponse(NSM_SUCCESS, 0)))
        .WillOnce(mockPostPatchIO(createInstallTokenResponse(NSM_SUCCESS, 0)))
        .WillRepeatedly(mockPostPatchIO(NSM_SUCCESS));
...
TEST_F(NsmDebugTokenUnifiedMultiRecordTest, PartialFailureOnMiddleRecord)
{
    ...
    // Record 1 succeeds; record 2's only chunk fails at the wire level;
    // record 3 succeeds. queryTokenHandler then fires once because
    // okCount > 0.
    EXPECT_CALL(*mockDevice, postPatchIO)
        .WillOnce(mockPostPatchIO(createInstallTokenResponse(NSM_SUCCESS, 0)))
