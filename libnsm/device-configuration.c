    ...
    uint16_t *reason_code, uint8_t *data, size_t data_size_max)
{
	...
	*data_size = le16toh(resp->hdr.data_size);
	...
	if ((size_t)*data_size > data_size_max) {
		return NSM_SW_ERROR_LENGTH;
	}
					...
					uint8_t *power_supply_status)
{
	...
	int ret = decode_get_fpga_diagnostics_settings_resp(
	    ...
	    (uint8_t *)power_supply_status, sizeof(uint8_t));
	if (ret != NSM_SW_SUCCESS || *cc != NSM_SUCCESS)
		return ret;
	if (data_size < sizeof(uint8_t))
		ret = NSM_SW_ERROR_LENGTH;
				 ...
				 uint8_t *mode)
{
	uint16_t data_size = 0;
	int ret = decode_get_fpga_diagnostics_settings_resp(
	    msg, msg_len, cc, &data_size, reason_code, mode, sizeof(uint8_t));
	if (ret != NSM_SW_SUCCESS || *cc != NSM_SUCCESS)
		return ret;
	if (data_size < sizeof(uint8_t))
		ret = NSM_SW_ERROR_LENGTH;
					  ...
					  struct nsm_msg *msg)
{
	...
	request->command = NSM_GET_SUPPORTED_DEVICE_MODES_V2;
	request->data_size = 0;
					   ...
					   struct nsm_msg *msg)
{
	...
	resp->hdr.command = NSM_GET_SUPPORTED_DEVICE_MODES_V2;
	resp->hdr.completion_code = cc;
	resp->handle = htole16(handle);
	resp->mode_count = htole16(mode_count);

	uint16_t data_size = sizeof(resp->handle) + sizeof(resp->mode_count) +
			     mode_count * sizeof(resp->supported_mode_list[0]);
	resp->hdr.data_size = htole16(data_size);

	for (uint16_t i = 0; i < mode_count; i++) {
		resp->supported_mode_list[i] = htole32(supported_mode_list[i]);
	}
					   ...
					   uint32_t *supported_mode_list)
{
	...
	*handle = le16toh(response->handle);
	*mode_count = le16toh(response->mode_count);

	uint16_t expected_data_size =
	    sizeof(response->handle) + sizeof(response->mode_count) +
	    (*mode_count) * sizeof(response->supported_mode_list[0]);

	if (le16toh(response->hdr.data_size) != expected_data_size) {
		return NSM_SW_ERROR_LENGTH;
	}

	if (*mode_count > 0 && supported_mode_list != NULL) {
		for (uint16_t i = 0; i < *mode_count; i++) {
			supported_mode_list[i] =
			    le32toh(response->supported_mode_list[i]);
		}
	}
...
/* --- Get/Set Device Config (Type 5, commands 0x10, 0x11) — V2 header --- */
...
int encode_set_device_config_v2_req(uint8_t instance_id,
				    ...
				    struct nsm_msg *msg)
{
	if (msg == NULL || (data == NULL && data_length > 0)) {
		return NSM_SW_ERROR_NULL;
	}

	struct nsm_set_device_config_v2_req *request =
	    (struct nsm_set_device_config_v2_req *)msg->payload;

	if ((uint32_t)sizeof(request->device_config_type) +
	...
	request->hdr.command = NSM_SET_DEVICE_CONFIG_V2;
	request->hdr.data_size =
	    htole16(sizeof(request->device_config_type) + data_length);
	request->device_config_type = htole32(device_config_type);
...
{
	if (msg == NULL || device_config_type == NULL || data_length == NULL) {
		return NSM_SW_ERROR_NULL;
	}
	...
	if (request->hdr.command != NSM_SET_DEVICE_CONFIG_V2) {
		return NSM_SW_ERROR_DATA;
	}

	uint16_t payload_size = le16toh(request->hdr.data_size);
	if (payload_size < sizeof(request->device_config_type)) {
		return NSM_SW_ERROR_DATA;
	}

	*device_config_type = le32toh(request->device_config_type);
	*data_length = payload_size - sizeof(request->device_config_type);

	if ((size_t)sizeof(struct nsm_msg_hdr) +
		sizeof(struct nsm_common_req_v2) + (size_t)payload_size >
	    msg_len) {
		return NSM_SW_ERROR_LENGTH;
	}
	...
	if (*data_length > 0) {
		memcpy(data, request->data, *data_length);
	}
...
{
	if (msg == NULL || (query_data == NULL && query_length > 0)) {
		return NSM_SW_ERROR_NULL;
	}

	struct nsm_get_device_config_v2_req *request =
	    (struct nsm_get_device_config_v2_req *)msg->payload;
	...
	request->hdr.command = NSM_GET_DEVICE_CONFIG_V2;
	request->hdr.data_size =
	    htole16(sizeof(request->device_config_type) + query_length);
	request->device_config_type = htole32(device_config_type);

	if (query_length > 0) {
		memcpy(request->device_config_query, query_data, query_length);
	}
...
int decode_get_device_config_v2_req(const struct nsm_msg *msg, size_t msg_len,
				    uint32_t *device_config_type,
				    uint8_t *query_data, uint16_t *query_length)
{
	if (msg == NULL || device_config_type == NULL || query_length == NULL) {
		return NSM_SW_ERROR_NULL;
	}
	...
	if (request->hdr.command != NSM_GET_DEVICE_CONFIG_V2) {
		return NSM_SW_ERROR_DATA;
	}
	...
	*device_config_type = le32toh(request->device_config_type);
	*query_length = payload_size - sizeof(request->device_config_type);
	...
	if (*query_length > 0 && query_data == NULL) {
		return NSM_SW_ERROR_NULL;
	}

	if (*query_length > 0) {
		memcpy(query_data, request->device_config_query, *query_length);
	}
				     ...
				     struct nsm_msg *msg)
{
	...
	if (cc != NSM_SUCCESS) {
		return encode_reason_code(cc, reason_code,
					  NSM_GET_DEVICE_CONFIG_V2, msg);
	}

	if (current_config_data == NULL && current_config_length > 0) {
		return NSM_SW_ERROR_NULL;
	}
	if (pending_config_data == NULL && pending_config_length > 0) {
		return NSM_SW_ERROR_NULL;
	}
	{
		uint32_t fixed =
		    (uint32_t)sizeof(uint16_t) + (uint32_t)sizeof(uint16_t);
		uint32_t total = fixed + (uint32_t)current_config_length +
				 (uint32_t)pending_config_length;
		if (total > UINT16_MAX) {
			return NSM_SW_ERROR_LENGTH;
		}
	}

	struct nsm_get_device_config_v2_resp *resp =
	    (struct nsm_get_device_config_v2_resp *)msg->payload;

	resp->hdr.command = NSM_GET_DEVICE_CONFIG_V2;
	resp->hdr.completion_code = cc;
	resp->current_config_length = htole16(current_config_length);
	resp->pending_config_length = htole16(pending_config_length);

	uint16_t data_size =
	    (uint16_t)((uint32_t)sizeof(resp->current_config_length) +
		       (uint32_t)sizeof(resp->pending_config_length) +
		       (uint32_t)current_config_length +
		       (uint32_t)pending_config_length);
	resp->hdr.data_size = htole16(data_size);

	if (current_config_length > 0) {
		memcpy(resp->config_data, current_config_data,
		       current_config_length);
	}
	if (pending_config_length > 0) {
		memcpy(resp->config_data + current_config_length,
		       pending_config_data, pending_config_length);
	}
...
{
	if (msg == NULL || cc == NULL || reason_code == NULL ||
	    current_config_length == NULL || pending_config_length == NULL) {
		return NSM_SW_ERROR_NULL;
	}
	...
	if (msg_len < sizeof(struct nsm_msg_hdr) +
			  sizeof(struct nsm_get_device_config_v2_resp) -
			  sizeof(uint8_t)) {
		return NSM_SW_ERROR_LENGTH;
	}
	...
	*current_config_length = le16toh(resp->current_config_length);
	*pending_config_length = le16toh(resp->pending_config_length);

	uint32_t expected_data_size =
	    (uint32_t)sizeof(resp->current_config_length) +
	    (uint32_t)sizeof(resp->pending_config_length) +
	    (uint32_t)*current_config_length + (uint32_t)*pending_config_length;
	if (expected_data_size > UINT16_MAX) {
		return NSM_SW_ERROR_LENGTH;
	}
	if (le16toh(resp->hdr.data_size) != expected_data_size) {
		return NSM_SW_ERROR_LENGTH;
	...
	if (sizeof(struct nsm_msg_hdr) +
		offsetof(struct nsm_get_device_config_v2_resp, config_data) +
		(size_t)*current_config_length +
		(size_t)*pending_config_length >
	    msg_len) {
		return NSM_SW_ERROR_LENGTH;
	}
	if (*current_config_length > 0 && current_config_data == NULL) {
		return NSM_SW_ERROR_NULL;
	}
	if (*pending_config_length > 0 && pending_config_data == NULL) {
		return NSM_SW_ERROR_NULL;
	}

	if (*current_config_length > 0) {
		memcpy(current_config_data, resp->config_data,
		       *current_config_length);
	}
	if (*pending_config_length > 0) {
		memcpy(pending_config_data,
		       resp->config_data + *current_config_length,
		       *pending_config_length);
	}
					      ...
					      struct nsm_msg *msg)
{
	...
	return encode_nsm_event(instance_id, NSM_TYPE_DEVICE_CONFIGURATION,
...
{
	if (msg == NULL || event_state == NULL) {
		return NSM_SW_ERROR_NULL;
	}
	if (msg->hdr.nvidia_msg_type != NSM_TYPE_DEVICE_CONFIGURATION) {
		return NSM_SW_ERROR_DATA;
	}

	uint8_t data_size = 0;
	int result = decode_nsm_event(
	    msg, msg_len, NSM_DEVICE_CONFIGURATION_REQUEST_EVENT_V1,
