...
struct instance_db {
	struct eid_state state[MCTP_EID_MAX];
	int lock_db_fd;
};

static inline int iid_next(instance_id_t cur)
{
	return (cur + 1) % INST_ID_MAX;
}

int instance_db_init(struct instance_db **ctx, const char *dbpath)
{
	struct instance_db *l_ctx;
	struct stat statbuf;
	int rc;
	...
	if (statbuf.st_size < ((off_t)(MCTP_EID_MAX) * (off_t)(INST_ID_MAX))) {
		return -EINVAL;
	}

	l_ctx = calloc(1, sizeof(struct instance_db));
	if (!l_ctx) {
		return -ENOMEM;
	}

	/* Initialise previous ID values so the next one is zero */
	for (int i = 0; i < MCTP_EID_MAX; i++) {
...
{
	return instance_db_init(ctx, "/usr/share/libnsm/instance-db/default");
}
		      ...
		      instance_id_t *iid)
{
	...
	l_iid = ctx->state[eid].prev;
	if (l_iid >= INST_ID_MAX) {
		return -EPROTO;
	}

	while ((l_iid = iid_next(l_iid)) != ctx->state[eid].prev) {
		struct flock flop;
		off_t loff;
		int rc;

		/* Have we already allocated this instance ID? */
		if (ctx->state[eid].allocations & BIT(l_iid)) {
			continue;
		}

		/* Derive the instance ID offset in the lock database */
		loff = eid * INST_ID_MAX + l_iid;
		...
		if (flop.l_type == F_UNLCK) {
			ctx->state[eid].prev = l_iid;
			ctx->state[eid].allocations |= BIT(l_iid);
			*iid = l_iid;
			return 0;
		}
...
int instance_id_free(struct instance_db *ctx, mctp_eid_t eid, instance_id_t iid)
{
	...
	if (!(ctx->state[eid].allocations & BIT(iid))) {
		return -EINVAL;
	}

	flop = cflu;
	flop.l_start = eid * INST_ID_MAX + iid;
	rc = fcntl(ctx->lock_db_fd, F_OFD_SETLK, &flop);
	if (rc < 0) {
		if (errno == EAGAIN || errno == EINTR) {
			return -EAGAIN;
	...
	ctx->state[eid].allocations &= ~BIT(iid);
