...
/** @brief Metric type that triggered the transition to Attention state
 *
 *  Values match the metric_type encoding of the PDDR register in the GPU PRM,
 *  which is authoritative here. The NSM spec table lists a different order and
 *  is incorrect.
 */
enum nsm_attention_trigger {
	...
	NSM_ATTENTION_TRIGGER_RAW_BER = 5,
