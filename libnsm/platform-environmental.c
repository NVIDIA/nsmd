...
int encode_set_device_power_limit_req(uint8_t instance, uint8_t action,
			       ...
			       struct nsm_msg *msg)
{
	return encode_set_power_limit_req(instance, DEVICE, action, persistence, power_limit, msg);
}
...
int encode_set_module_power_limit_req(uint8_t instance, uint8_t action,
			       ...
			       struct nsm_msg *msg)
{
	return encode_set_power_limit_req(instance, MODULE, action, persistence, power_limit, msg);
}

int decode_set_power_limit_req(const struct nsm_msg *msg, size_t msg_len, uint32_t* id,
			       uint8_t *action, uint8_t *persistence,
			       uint32_t *power_limit)
{
	if (msg == NULL || id == NULL || action == NULL || persistence == NULL ||
	    power_limit == NULL) {
		return NSM_SW_ERROR_NULL;
	}
	...
	if (request->hdr.data_size != 2 * sizeof(uint8_t) + 2* sizeof(uint32_t)) {
		return NSM_SW_ERROR_DATA;
	}
