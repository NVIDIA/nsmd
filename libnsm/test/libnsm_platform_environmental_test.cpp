...
TEST(encode_get_power_supply_status_req, testGoodEncodeRequest)
{
	std::vector<uint8_t> requestMsg(sizeof(nsm_msg_hdr) +
					sizeof(nsm_common_req));
	...
	auto rc = encode_get_power_supply_status_req(0, request);
	...
	EXPECT_EQ(NSM_GET_POWER_SUPPLY_STATUS, req->command);
	EXPECT_EQ(0, req->data_size);
}

TEST(decode_get_power_supply_status_req, testGoodDecodeRequest)
{
	std::vector<uint8_t> requestMsg{
	    0x10,
	    0xDE,			     // PCI VID: NVIDIA 0x10DE
	    ...
	    0x89,			     // OCP_TYPE=8, OCP_VER=9
	    NSM_TYPE_PLATFORM_ENVIRONMENTAL, // NVIDIA_MSG_TYPE
	    NSM_GET_POWER_SUPPLY_STATUS,     // command
	    0,				     // data size
	};
	...
	auto rc = decode_get_power_supply_status_req(request, msg_len);

	EXPECT_EQ(rc, NSM_SW_SUCCESS);
}

TEST(encode_get_power_supply_status_resp, testGoodEncodeResponse)
{
	std::vector<uint8_t> responseMsg(
...
TEST(encode_get_gpu_presence_and_power_status_req, testGoodEncodeRequest)
{
	...
	auto rc = encode_get_gpu_presence_and_power_status_req(0, request);
	...
	EXPECT_EQ(NSM_GET_GPU_PRESENCE_POWER_STATUS, req->command);
	EXPECT_EQ(0, req->data_size);
}

TEST(decode_get_gpu_presence_and_power_status_req, testGoodDecodeRequest)
{
	std::vector<uint8_t> requestMsg{
	    0x10,
	    0xDE,			     // PCI VID: NVIDIA 0x10DE
	    ...
	    0x89,			     // OCP_TYPE=8, OCP_VER=9
	    NSM_TYPE_PLATFORM_ENVIRONMENTAL, // NVIDIA_MSG_TYPE
	    NSM_GET_GPU_PRESENCE_POWER_STATUS, // command
	    0,				       // data size
	};
	...
	auto rc =
	    decode_get_gpu_presence_and_power_status_req(request, msg_len);

	EXPECT_EQ(rc, NSM_SW_SUCCESS);
}

TEST(encode_get_gpu_presence_and_power_status_resp, testGoodEncodeResponse)
{
	std::vector<uint8_t> responseMsg(
	    sizeof(nsm_msg_hdr) +
	...
	uint8_t power_status = 0x01;
	uint8_t presence = 0x01;
	uint16_t reasonCode = ERR_NULL;

	auto rc = encode_get_gpu_presence_and_power_status_resp(
	    0, NSM_SUCCESS, reasonCode, presence, power_status, response);
	...
	EXPECT_EQ(sizeof(uint32_t), le16toh(resp->hdr.data_size));
	EXPECT_EQ(power_status, le32toh(resp->power_status));
	EXPECT_EQ(presence, le32toh(resp->presence));
}
...
#define EXPECTED_POWER 0x08
	...
	auto rc = decode_get_gpu_presence_and_power_status_resp(
	    response, msg_len, &cc, &reasonCode, &presence, &power);
...
{
	std::vector<uint8_t> responseMsg(
	    sizeof(nsm_msg_hdr) + sizeof(struct nsm_get_current_clock_event_reason_code_resp), 0);
	auto response = reinterpret_cast<nsm_msg *>(responseMsg.data());
	bitfield32_t flags;
	flags.byte = 3;
	uint16_t reason_code = ERR_NULL;

	auto rc = encode_get_current_clock_event_reason_code_resp(0, NSM_SUCCESS, reason_code, &flags,
					   response);

	struct nsm_get_current_clock_event_reason_code_resp *resp =
	    reinterpret_cast<struct nsm_get_current_clock_event_reason_code_resp *>(response->payload);
...
TEST(getClockEventReason, testGoodDecodeResponse)
{
	...
	auto rc = decode_get_current_clock_event_reason_code_resp(response, msg_len, &cc, &data_size,
					   &reason_code, &flags);
...
TEST(getClockEventReason, testBadDecodeResponse)
{
	...
	auto rc = decode_get_current_clock_event_reason_code_resp(NULL, msg_len, &cc, &data_size,
					   &reason_code, &flags);
	EXPECT_EQ(rc, NSM_SW_ERROR_NULL);

	rc = decode_get_current_clock_event_reason_code_resp(response, msg_len, NULL, &data_size,
				      &reason_code, &flags);
	EXPECT_EQ(rc, NSM_SW_ERROR_NULL);

	rc = decode_get_current_clock_event_reason_code_resp(response, msg_len, &cc, NULL,
				      &reason_code, &flags);
	EXPECT_EQ(rc, NSM_SW_ERROR_NULL);

	rc = decode_get_current_clock_event_reason_code_resp(response, msg_len - 1, &cc, &data_size,
				      &reason_code, &flags);
	EXPECT_EQ(rc, NSM_SW_ERROR_LENGTH);

	rc = decode_get_current_clock_event_reason_code_resp(response, msg_len, &cc, &data_size,
				      &reason_code, &flags);
	EXPECT_EQ(rc, NSM_SW_ERROR_DATA);
}
